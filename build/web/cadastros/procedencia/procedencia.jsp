...
<%@page import="br.com.controlgames.model.Procedencia"%>
    ...
    <body>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Procedencias</li>
            </ol>
        </nav>
        ...
        <div class="container my-3">
            <table id="tabdados" name="tbusuario" class="table rounded">
                ...
                <%
                    List<Procedencia> Procedencias = (List<Procedencia>) request.getAttribute("procedencias");
                    for (Procedencia procedencia : Procedencias) {
                %>
                <tbody>
                    <tr scope="row">
                        <td><%= procedencia.getIdProcedencia()%></td>
                        <td><%= procedencia.getDescricao()%></td>
                        <td><%= procedencia.getSituacao()%></td>
                        <%
                            if (procedencia.getSituacao().equals("A")) {
                        %>
                        <td><a class="btn btn-danger" href="${pageContext.request.contextPath}/ProcedenciaExcluir?idProcedencia=<%=procedencia.getIdProcedencia()%>">Inativar</a>
                            <%
                            } else {
                            %>
                        <td><a class="btn btn-success" href="${pageContext.request.contextPath}/ProcedenciaExcluir?idProcedencia=<%=procedencia.getIdProcedencia()%>">Ativar</a>
                            <%
                                }
                            %>
                        <td><a class="btn btn-outline-success" href="${pageContext.request.contextPath}/ProcedenciaCarregar?idProcedencia=<%=procedencia.getIdProcedencia()%>">Alterar</a>
                    </tr>
                    <%
                        }
                    %>
        ...
        <div class="modal fade" id="CadModal" tabindex="-1" role="dialog" aria-labelledby="CadModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form name="cadastrarUsuario" action="${pageContext.request.contextPath}/ProcedenciaCadastrar" method="POST">
